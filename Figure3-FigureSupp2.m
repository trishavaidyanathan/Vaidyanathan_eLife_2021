...
% DREADD CONTROLS
path = 'E:\Analysis\MAT files\MAT Files for Paper 2020\DATA FOR DRYAD\DREADDs\DREADD Controls'; %change this to your path on your computer

%Load Gi-DREADD Controls 
load(strcat(path,'\ephysData'))
load(strcat(path,'\aquaData'))
...
m = 5; %example chosen was m166, first hour
    figure;
    ha(1) = subplot(2,1,1);
...
%% Fig3FigSupp2 F
    ...
    f=1; %corresponds to spec select
    n=1; %!!!! 2=sleep, change to 1 if you want wake! 
    mindur = 10; %min duration of sleep period
    allDelta_cnoNorm = {}; allDelta_salNorm = {};
    miceToUse = intersect(hasbaseline,lfpMice); 
...
%%% Fig3FigSupp2 G LEFT, %-time sleeping
...
%%% Fig3FigSupp2 G MIDDLE, sleep bout length
...
figure;
    ...
    colors = jet(length(miceToUse));
    for m = miceToUse
        s(k)=scatter([1 2], [mean(sLengths_sal{m}) mean(sLengths_cno{m})],80,'filled');
        hold on;
        errorbar([1 2], [mean(sLengths_sal{m}) mean(sLengths_cno{m})], [0 0],'k');
        hold on;
        k=k+1;
    end
    sal = cellfun(@mean, sLengths_sal(miceToUse));
    cno = cellfun(@mean, sLengths_cno(miceToUse));
    scatter([0.75 2.25], [mean(sal) mean(cno)],'k','filled')
    hold on;
    errorbar([0.75 2.25], [mean(sal) mean(cno)],[std(sal) std(cno)],...
        'k','linestyle','none')
    [h,p] = ttest(cellfun(@mean,sLengths_sal(miceToUse)), cellfun(@mean, sLengths_cno(miceToUse)));
    hold on;
    text(1.2, max([sal cno]), strcat('p =',{' '},num2str(p)))
    xlim([0 3]);
    xticks([1 2])
    xticklabels({'saline','CNO'}); 
    ylabel('sleep bout length'); 
    title(strcat(allData(1).dreadd,{' '},'All hours'));
    ylim([0 500])
    set(gcf,'renderer','painters')
    set(gca,'fontsize',15)

%%% Fig3FigSupp2 G RIGHT, num of sleep bouts
...
figure;
    ...
    colors = jet(length(miceToUse));
    for m = miceToUse
        s(k)=scatter([1 2], [mean(numS_sal{m}) mean(numS_cno{m})],80,'filled');
        hold on;
        errorbar([1 2], [mean(numS_sal{m}) mean(numS_cno{m})], [0 0],'k');
        hold on;
        k=k+1;
    end
    sal = cellfun(@mean, numS_sal(miceToUse));
    cno = cellfun(@mean, numS_cno(miceToUse));
    scatter([0.75 2.25], [mean(sal) mean(cno)],'k','filled')
    hold on;
    errorbar([0.75 2.25], [mean(sal) mean(cno)],[std(sal) std(cno)],...
        'k','linestyle','none')
    [h,p] = ttest(cellfun(@mean,numS_sal(miceToUse)), cellfun(@mean, numS_cno(miceToUse)));
    hold on;
    text(1.2, max([sal cno]), strcat('p =',{' '},num2str(p)))
    xlim([0 3]);
    ylim([0 60]);
...
%%% Fig3FigSupp2 H LEFT, %-time waking
...
%%% Fig3FigSupp2 H MIDDLE, wake bout length
...
figure;
    ...
    xticks([1 2])
    xticklabels({'saline','CNO'}); 
    ylabel('wake bout length'); 
    title(strcat(allData(1).dreadd,{' '},'All hours'));
    ylim([0 500])
    set(gcf,'renderer','painters')
    set(gca,'fontsize',15)

%%% Fig3FigSupp2 H RIGHT, num of wake bouts
